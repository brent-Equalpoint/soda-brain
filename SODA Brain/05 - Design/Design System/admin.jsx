...
function AButton({ children, variant = 'primary', onClick, disabled, icon, size = 'md', style }) {
  ...
  const sz = { sm: { padding: '7px 12px', fontSize: 13 }, md: { padding: '11px 16px', fontSize: 14 } }[size];
  return <button onClick={disabled ? undefined : onClick} disabled={disabled} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontFamily: 'var(--font-sans)', fontWeight: 600, borderRadius: 'var(--r-md)', cursor: disabled ? 'default' : 'pointer', opacity: disabled ? .5 : 1, ...sz, ...v, ...(style || {}), width: "170px", margin: "0px" }}>{icon && <span aria-hidden="true">{icon}</span>}{children}</button>;
}
