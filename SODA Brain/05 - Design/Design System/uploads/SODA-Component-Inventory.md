...
A note on status. The pieces below are grounded in the actual prototype screens that exist now: the survey, the survey flow, the creative sync screen, the room demo, the admin panel, and the command center. Where a component is designed but not yet wired into a built screen, it says so. Per decision SODA-032, the vocabulary is fixed: Built means present in the production-bound kit, Prototyped means rendered only in a simulation, and Specified means designed on paper and waiting.
...
**Status.** Prototyped. Rendered in the admin simulation only, not yet in the production kit.
...
# Part Four: The Home Area

These are the screens of the home, where a guest lives between events: their profile, their event history, and the contacts they have met or matched with. They are a tabbed home with a profile header and three tabs, Overview, Events, and Contacts. The Follow-Up Nudge is the timed prompt to reach out, driven by contact warmth.

## 74. The Home Shell

**What it is.** The frame for the between-events home: a compact profile header on top and the three tabs at the bottom.

**What the person sees.** A circular avatar, the guest name and role, a settings affordance, and a tab bar with Overview, Events, and Contacts. The active tab is green, and a purple dot on Contacts signals waiting follow-ups.

**What it connects to.** The container for the three home tabs. The profile header taps into the full profile, a detail screen specified later.

**Status.** Specified. Defined in the Home Screen spec.
...
The active tab is Soda green #3BD75C, the others muted grey. A small purple #A47BFF dot on Contacts.
8px grid, 16px margins, 44px touch targets.
```

## 75. The Overview Tab

**What it is.** The personalized landing of the home, the first thing seen on opening the app between events.

**What the person sees.** A greeting, a compact strip of recent events that taps through to the Events tab, a dominant block of people you synced with, and an experience insight card drawn from survey feedback.

**What it connects to.** The default home tab. Routes to the Events and Contacts tabs.
...
Top: the compact profile header. Greeting: a bold condensed uppercase headline, white, WELCOME BACK.
Recent events: a horizontal strip of small cards with host, title, date, and a met count, linking to
the Events tab. People you synced with, the dominant block: a row of avatars with names. Your
experience: a calm deep green #203229 insight card drawn from the survey.
Bottom: the three-tab bar with Overview active in Soda green #3BD75C. 8px grid, 16px margins.
```

## 76. The Events Tab

**What it is.** The full history of events the guest has attended, with where and when.

**What the person sees.** A list of events newest first, each row with the host identity, title, date, the location where it was held, and a count of people met. The location fills automatically from the event and can be edited by hand.

**What it connects to.** A home tab. Each row taps into an event detail, specified later. Carries the location history.
...
Body, dominant: a vertical list of event rows, each with a host logo or name, an event title in white,
a date and location in muted grey monospace ("Mar 14, Cleveland"), and a small met count on the right.
Each row tappable. Bottom: three-tab bar with Events active in Soda green #3BD75C.
8px grid, 16px margins, 44px touch targets.
```

## 77. The Contacts Tab, the Rolodex

**What it is.** Every person the guest has met, and where they are in following up. The heart of the home.

**What the person sees.** A dismissible overview of recently met people with three signals, saved, reached out in green, and a purple follow-up nudge, sitting on top of the full searchable rolodex. Each row carries a met-at line and a reserved share action.

**What it connects to.** A home tab. The dominant element of the home. Reads the contact warmth and follow-up nudge below.
...
SODA home, the rolodex. One job: show everyone met and where they are in following up.
Top: the compact profile header with a small "+" or scan icon reserved for adding contacts.
A dismissible "RECENTLY MET & FOLLOW-UPS" section: recent contact rows with a signal each, a muted
"Saved" tag, a Soda green #3BD75C check for "Reached out", or a purple #A47BFF dot for "Reach out".
Beneath it, the dominant element: the full rolodex, a search field then a scrollable list of contact
rows, each with avatar, name, role, a met-at line in monospace, and a small action icon.
Bottom: three-tab bar with Contacts active, a purple dot on the tab. Green confirms, purple nudges.
```

## 78. The Follow-Up Nudge

**What it is.** The timed prompt to reach out to a contact, the relationship intelligence of the platform applied to the guest's own contacts.

**What the person sees.** A purple nudge on a contact the guest has not reached out to, surfacing as warmth decays. Saved or matched contacts surface sooner. Frequency is capped, and it never nags.

**What it connects to.** Reads each contact's warmth, which decays on the platform curve. SODA suggests but never messages on the guest's behalf, and a dismissed nudge is logged as a signal.

**Status.** Specified. Defined in the Home Screen spec technicals.
