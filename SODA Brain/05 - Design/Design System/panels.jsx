...
/* Event creation + host identity (draft) */
function EventSetup({ onToast }) {
  ...
  return (
    <APanel title="Event setup & host identity">
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: '0 18px' }}>
        <AField label="Event name · required"><input value={name} onChange={(e) => setName(e.target.value)} style={aInput} /></AField>
        <AField label="Host name · required"><input value={host} onChange={(e) => setHost(e.target.value)} style={aInput} /></AField>
        <AField label="Date · required"><input defaultValue="Mar 14, 2026 · 7:00pm" style={aInput} /></AField>
        <AField label="Host logo · optional">
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, ...{} }}>
            <div style={{ flex: 1, ...aInput, display: 'flex', alignItems: 'center', color: 'var(--text-faint)', fontSize: "10px" }}>No logo — host name shown in type</div>
            <AButton variant="ghost" size="sm" icon="↑">Upload</AButton>
          </div>
        </AField>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 6 }}>
        <AButton onClick={() => onToast && onToast('Event saved as draft')}>Save draft</AButton>
        <AButton variant="ghost" onClick={() => onToast && onToast('Preview opened')}>Preview attendee view</AButton>
      </div>
    </APanel>);
